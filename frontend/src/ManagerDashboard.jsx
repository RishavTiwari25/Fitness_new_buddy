    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#1F1E1D', padding: '24px' }}>
      ...
      <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
        ...
        <div style={{ width: 340, position: 'sticky', top: 24 }}>
          <div style={{ backgroundColor: '#262624', border: '1px solid #3A3937', borderRadius: 16, padding: 16, marginBottom: 16 }}>
            <h4 style={{ color: '#F5F4EE', marginBottom: 10 }}>Create a Gym</h4>
            <form onSubmit={createGym}>
              <div>
                <input placeholder="Gym name *" value={name} onChange={e => setName(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: '#1F1E1D', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 10 }} required />
              </div>
              <div style={{ marginTop: 8 }}>
                <input placeholder="Location" value={location} onChange={e => setLocation(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: '#1F1E1D', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 10 }} />
              </div>
              <div style={{ marginTop: 8 }}>
                <textarea placeholder="Description" value={newDesc} onChange={e => setNewDesc(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: '#1F1E1D', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 10, resize: 'vertical' }} rows={2} />
              </div>
              <div style={{ marginTop: 8 }}>
                <input placeholder="Timings (e.g., 6 AM - 10 PM)" value={newTimings} onChange={e => setNewTimings(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: '#1F1E1D', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 10 }} />
              </div>
              <div style={{ marginTop: 8 }}>
                <input placeholder="Contact Info" value={newContact} onChange={e => setNewContact(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: '#1F1E1D', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 10 }} />
              </div>
              <div style={{ marginTop: 8 }}>
                <input placeholder="Amenities (comma separated)" value={newAmenities} onChange={e => setNewAmenities(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: '#1F1E1D', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 10 }} />
              </div>
              <div style={{ marginTop: 10 }}>
                <button type="submit" style={{ backgroundColor: '#D97757', color: '#FFFFFF', borderRadius: 9999, padding: '10px 16px', border: 'none', cursor: 'pointer', fontWeight: 700, width: '100%' }}>Create Gym</button>
              </div>
            </form>
          </div>

          <div style={{ backgroundColor: '#262624', border: '1px solid #3A3937', borderRadius: 16, padding: 16 }}>
            <h4 style={{ color: '#F5F4EE', marginBottom: 10 }}>Your Gyms</h4>
            <div>
              {gyms.map(g => (
                <div key={g.id} style={{ padding: 6 }}>
                  <button onClick={() => loadEquipment(g.id)} style={{ padding: '8px 12px', borderRadius: 9999, border: '1px solid #3A3937', background: selectedGym === g.id ? '#D97757' : '#3A3937', color: selectedGym === g.id ? '#1F1E1D' : '#F5F4EE', fontWeight: 700, cursor: 'pointer' }}>
                    {g.name} {g.location ? `(${g.location})` : ''}
                  </button>
                </div>
              ))}
              {gyms.length === 0 && <p style={{ color: '#A6A29A' }}>No gyms yet.</p>}
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
          {!selectedGym && (
            <div style={{ background: '#262624', border: '1px solid #3A3937', borderRadius: 16, padding: 24, color: '#A6A29A' }}>
            ...
            <>
              ...
              <div style={{ backgroundColor: '#262624', border: '1px solid #3A3937', borderRadius: 16, padding: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                  <h3 style={{ color: '#F5F4EE', margin: 0 }}>Overview — Gym #{selectedGym}</h3>
                </div>
                <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 260 }}>
                    <div style={{ color: '#A6A29A', marginBottom: 6, fontWeight: 600 }}>Currently inside</div>
                    <div style={{ background: '#1F1E1D', border: '1px solid #3A3937', borderRadius: 12, padding: 12, maxHeight: 220, overflow: 'auto' }}>
                      ...
                      {present.map(m => (
                        <div key={m.presence_id} style={{ fontSize: 14, padding: '6px 0', borderBottom: '1px solid #2e2e31' }}>
                          <div><strong style={{ color: '#F5F4EE' }}>{m.name || m.email}</strong></div>
                          <div style={{ color: '#A6A29A' }}>{m.email}</div>
              ...
              {/* Section: Gym Profile */}
              {activeTab === 'overview' && (
              <div style={{ backgroundColor: '#262624', border: '1px solid #3A3937', borderRadius: 16, padding: 16, marginTop: 16 }}>
                <h3 style={{ color: '#F5F4EE', marginTop: 0 }}>Gym Profile</h3>
                {profMsg && <div style={{ marginBottom: 12, color: profMsg.includes('success') ? '#D97757' : '#ef4444' }}>{profMsg}</div>}
                <form onSubmit={saveGymProfile} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div>
                    <label style={{ display: 'block', color: '#A6A29A', fontSize: 14, marginBottom: 4 }}>Description</label>
                    <textarea placeholder="e.g., A state of the art facility..." value={profDesc} onChange={e => setProfDesc(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: '#1F1E1D', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 10, minHeight: 60, fontFamily: 'inherit' }} />
                  </div>
                  <div style={{ display: 'flex', gap: 12 }}>
                    <div style={{ flex: 1 }}>
                      <label style={{ display: 'block', color: '#A6A29A', fontSize: 14, marginBottom: 4 }}>Timings</label>
                      <input placeholder="e.g., 6:00 AM - 10:00 PM" value={profTimings} onChange={e => setProfTimings(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: '#1F1E1D', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 10 }} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <label style={{ display: 'block', color: '#A6A29A', fontSize: 14, marginBottom: 4 }}>Contact Info</label>
                      <input placeholder="Phone / Email" value={profContact} onChange={e => setProfContact(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: '#1F1E1D', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 10 }} />
                    </div>
                  </div>
                  <div>
                    <label style={{ display: 'block', color: '#A6A29A', fontSize: 14, marginBottom: 4 }}>Amenities (comma separated)</label>
                    <input placeholder="e.g., WiFi, Showers, Lockers, Parking" value={profAmenities} onChange={e => setProfAmenities(e.target.value)} style={{ width: '100%', padding: '10px 12px', background: '#1F1E1D', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 10 }} />
                  </div>
                  <div>
                    <button type="submit" style={{ backgroundColor: '#D97757', color: '#FFFFFF', borderRadius: 9999, padding: '10px 16px', border: 'none', cursor: 'pointer', fontWeight: 700 }}>Save Profile</button>
                  </div>
                </form>
              </div>
              )}
              ...
              <>
              <div style={{ backgroundColor: '#262624', border: '1px solid #3A3937', borderRadius: 16, padding: 16 }}>
                <h3 style={{ color: '#F5F4EE', marginTop: 0 }}>Equipment</h3>
                <div>
                  {equipment.map(eq => <EquipmentRow key={eq.id} eq={eq} onEdit={startEdit} onDelete={deleteEquipment} />)}
                  {equipment.length === 0 && <p style={{ color: '#A6A29A' }}>No equipment.</p>}
                </div>

                <form onSubmit={editing ? saveEdit : addEquipment} style={{ marginTop: 12, display: 'grid', gridTemplateColumns: '1fr 1fr 120px', gap: 8 }}>
                  <input placeholder="Equipment name" value={eqName} onChange={e => setEqName(e.target.value)} style={{ padding: '10px 12px', background: '#1F1E1D', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 10 }} />
                  <input placeholder="Notes" value={eqNotes} onChange={e => setEqNotes(e.target.value)} style={{ padding: '10px 12px', background: '#1F1E1D', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 10 }} />
                  <div style={{ display: 'flex', gap: 8 }}>
                    <input type="number" min={1} value={eqQuantity} onChange={e => setEqQuantity(e.target.value)} style={{ width: 80, padding: '10px 12px', background: '#1F1E1D', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 10 }} />
                    <button type="submit" style={{ backgroundColor: '#D97757', color: '#FFFFFF', borderRadius: 9999, padding: '10px 16px', border: 'none', cursor: 'pointer', fontWeight: 700 }}>{editing ? 'Save' : 'Add'}</button>
                    {editing && <button type="button" onClick={() => { setEditing(null); setEqName(''); setEqNotes(''); setEqQuantity(1) }} style={{ backgroundColor: '#3A3937', color: '#F5F4EE', borderRadius: 9999, padding: '10px 16px', border: 'none', cursor: 'pointer', fontWeight: 700 }}>Cancel</button>}
                  </div>
                </form>
              </div>
              ...
              <div style={{ backgroundColor: '#262624', border: '1px solid #3A3937', borderRadius: 16, padding: 16, marginTop: 16 }}>
                <h3 style={{ color: '#F5F4EE', marginTop: 0 }}>Active Equipment Bookings</h3>
                <div>
                  ...
                  {bookings.map(b => (
                    <div key={b.booking_id} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #2e2e31' }}>
                      <div>
                        <strong style={{ color: '#F5F4EE' }}>{b.equipment_name}</strong>
                        <div style={{ fontSize: 12, color: '#A6A29A' }}>by {b.user_name || b.user_email} since {b.started_at}</div>
                      </div>
                      <div>
              ...
              {activeTab === 'members' && (
              <div style={{ backgroundColor: '#262624', border: '1px solid #3A3937', borderRadius: 16, padding: 16 }}>
                <h3 style={{ color: '#F5F4EE', marginTop: 0 }}>Memberships & Payments</h3>
                {saveMsg && <div style={{ marginBottom: 6, color: '#A6A29A' }}>{saveMsg}</div>}
                <div>
                  {memberships.length === 0 && <div style={{ color: '#666' }}>No members in this gym yet.</div>}
                  {memberships.map(m => (
                    <div key={m.user_id} style={{ background: '#1F1E1D', border: '1px solid #3A3937', borderRadius: 12, padding: 20, marginBottom: 16 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
                        <div>
                          <div style={{ fontSize: 18, fontWeight: 700, color: '#F5F4EE', display: 'flex', alignItems: 'center', gap: 8 }}>
                            {m.name || m.email}
                          ...
                          <div style={{ fontSize: 14, color: '#A6A29A', marginTop: 2 }}>{m.email}</div>
                          {m.status !== 'pending' && m.last_payment_at && <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 6 }}>Last payment: {m.last_payment_at}</div>}
                        </div>
                        {m.status === 'pending' ? (
                          <button onClick={() => approveMembership(m.user_id)} style={{ background: '#D97757', color: '#FFFFFF', border: 'none', borderRadius: 8, padding: '10px 20px', cursor: 'pointer', fontWeight: 700, fontSize: 14 }}>Approve Request</button>
                        ) : (
                          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                            <button onClick={() => remind(m.user_id)} style={{ background: '#262624', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 8, padding: '8px 12px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}><Icon name="bell" size={13} style={{ verticalAlign: '-2px', marginRight: 5 }} />Remind</button>
                            <button onClick={() => { const amt = prompt('Amount paid', String(m.monthly_fee || '')); if (amt) recordPayment(m.user_id, amt) }} style={{ background: '#262624', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 8, padding: '8px 12px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}><Icon name="dollar" size={13} style={{ verticalAlign: '-2px', marginRight: 5 }} />Record Payment</button>
                            <button onClick={() => toggleViewPayments(m.user_id)} style={{ background: '#262624', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 8, padding: '8px 12px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>{openPaymentsFor[m.user_id] ? 'Hide History' : 'View History'}</button>
                          </div>
                        )}
                      </div>

                      {m.status !== 'pending' && (
                        <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px dashed #3A3937', display: 'flex', gap: 16, alignItems: 'flex-end', flexWrap: 'wrap' }}>
                          <div style={{ flex: 1, minWidth: 150 }}>
                            <label style={{ display: 'block', fontSize: 12, color: '#A6A29A', marginBottom: 6, fontWeight: 600 }}>Monthly Fee (₹)</label>
                            <input type="number" placeholder="Amount" defaultValue={m.monthly_fee || ''} onBlur={e => m._fee = e.target.value} style={{ width: '100%', padding: '10px 12px', background: '#262624', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 8 }} />
                          </div>
                          <div style={{ flex: 1, minWidth: 150 }}>
                            <label style={{ display: 'block', fontSize: 12, color: '#A6A29A', marginBottom: 6, fontWeight: 600 }}>Next Due Date</label>
                            <input type="date" defaultValue={m.next_due_date || ''} onBlur={e => m._due = e.target.value} style={{ width: '100%', padding: '10px 12px', background: '#262624', color: '#F5F4EE', border: '1px solid #3A3937', borderRadius: 8 }} />
                          </div>
                          <div>
                            ...
                            )} style={{ background: '#D97757', color: '#FFFFFF', border: 'none', borderRadius: 8, padding: '11px 24px', cursor: 'pointer', fontWeight: 700, fontSize: 14 }}>Save Changes</button>
                          </div>
                        </div>
                      )}

                      {openPaymentsFor[m.user_id] && (
                        <div style={{ padding: '12px', background: '#262624', borderRadius: 8, marginTop: 16 }}>
                          <div style={{ fontSize: 12, color: '#A6A29A', marginBottom: 4 }}>Recent payments:</div>
                          ...
                          {paymentsCache[m.user_id] && paymentsCache[m.user_id].length > 0 && (
                            <ul style={{ margin: 0, paddingLeft: 16 }}>
                              {paymentsCache[m.user_id].map(p => (
                                <li key={p.id} style={{ fontSize: 13, color: '#F5F4EE' }}>
          ...
          {activeTab === 'leaderboard' && (
            <div style={{ backgroundColor: '#262624', border: '1px solid #3A3937', borderRadius: 16, padding: 16 }}>
              <h3 style={{ color: '#F5F4EE', marginTop: 0 }}>Gym Streak Leaderboard</h3>
